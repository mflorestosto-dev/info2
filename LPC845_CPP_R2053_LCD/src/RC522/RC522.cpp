...
extern Gpio gpioNSS;
extern Gpio gpioNRST;
extern Gpio gpioMISO;
extern Gpio gpioMOSI;
extern Gpio gpioSCK;
...
{
    uint8_t serNum[11]; // Espacio para almacenar el número de serie formateado
    uint8_t status;
    uint8_t str[MAX_LEN];
    ...
    // Buscar tarjeta, devuelve el tipo de tarjeta
    status = MFRC522_Request(PICC_REQIDL, str);

    // Evitar conflicto y obtener el número de serie de la tarjeta
    status = MFRC522_Anticoll(str);

    if (status == MI_OK) // Si se leyó correctamente la tarjeta
    {
        // Convertir el número de serie a formato hexadecimal para enviarlo por UART
        sprintf((char *)serNum, "%02X %02X %02X %02X %02X", str[0], str[1], str[2], str[3], str[4]);

        // Enviar el número de serie por UART
        UART0_Send((uint8_t*)serNum, strlen((char *)serNum));

        // Mostrar el número de serie en la pantalla LCD, si es necesario


        // Verificación de coincidencia (opcional, solo para validación interna)
    ...
    // Colocar el lector en estado de espera
    MFRC522_Halt();
}
...
void showSerialNumber(void)
{
	...
	if (status == MI_OK)
	{
		...
		UART0_Send( serNum, strlen( (char*)serNum));
...
void setup(void)
{
	gpioNSS.Set(OFF);	//GPIO_Set( nss, OFF );
	//nss = 0;						//Activate the RFID reader
	gpioNRST.Set(ON);//GPIO_Set( nrst, ON );
...
void sendToSerialHEX(int32_t block, uint8_t status, uint8_t *str)
{
	...
	if(status == MI_OK)
	{
		UART0_Send((uint8_t*)string0, 0 );
		UART0_Send((uint8_t*)string1, 0 );
		UART0_PushTx('\r');
		UART0_PushTx('\n');
	}
}
