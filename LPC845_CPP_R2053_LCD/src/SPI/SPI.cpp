...
{
	gpioNSS.Set(ON);//GPIO_Set(nss, ON);
	//nss = ON;
}
...
void SPI_HW_Write(uint8_t Data)
{
	...
	for( i = 0; i < 8 ; i++ )
	{
		if( Data & ((uint8_t)(0x80) >> i))
			gpioMOSI.Set(ON);//GPIO_Set(mosi, ON);
		else
			gpioMOSI.Set(OFF);//GPIO_Set(mosi, OFF);

		gpioSCK.Set(ON);//GPIO_Set(sck, ON);

		SPI_Demora(SPI_TIEMPO);

		gpioSCK.Set(OFF);//GPIO_Set(sck, OFF);
...
uint8_t SPI_HW_Read(void)
{
	...
	for( i = 0; i < 8 ; i++ )
	{
		...
		if(gpioMISO.Read())//;GPIO_Read(miso))
			Data |= ((uint8_t)(0x80) >> i);
