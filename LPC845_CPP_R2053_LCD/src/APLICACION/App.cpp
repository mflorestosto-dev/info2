...
extern Gpio 	ledRojo;
extern Gpio 	valvPrelav;
extern Gpio 	valvEnjuague;
...
void prueba(void){
	uint32_t nivelAgua = waterSensor.readRawValue();
	char buf1[30];
	sprintf(buf1, "Medicion = %d", nivelAgua);
	UART0_Send((uint8_t*)buf1, strlen(buf1));
}
